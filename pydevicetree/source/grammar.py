...
arth_str = p.Forward()
arith_str_expr = p.nestedExpr(content=(p.OneOrMore(operator ^ integer ^ label_raw) ^ ternary_expr))

cell_array = p.Literal("<").suppress() + \
        p.ZeroOrMore(integer ^ arith_expr ^ arith_str_expr ^ string ^ reference ^ label_creation.suppress()) + \
        p.Literal(">").suppress()
...
ifdef_label = p.Word(p.alphanums + "_")
...
node_definition << (node_opener ^ node_reference_opener) + \
        p.OneOrMore(property_assignment ^ directive ^ node_definition) + \
        node_closer
